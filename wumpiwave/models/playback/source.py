...
class PlayableSource:
    ...
    def __post_init__(self) -> None:
        ...
        if self.expires_at is not None and self.expires_at.tzinfo is None:
            raise ValueError(
                "The stream expiration date must contain timezone information."
            )
    ...
    def remaining_lifetime(self) -> float | None:
        ...
        remaining_seconds: float = (self.expires_at - datetime.now(UTC)).total_seconds()

        return max(0.0, remaining_seconds)


__all__: tuple[str, ...] = ("PlayableSource",)
